plotMyMfuzz <- function  (eset, cl, mfrow = c(1, 1), colo, min.mem = 0, time.labels,
    ...
    Xwidth = 5, Xheight = 5, single = FALSE,spline=TRUE, only_center=FALSE,...)
{
    clusterindex <- cl[[3]]
    memship <- cl[[4]]
    ...
    for (j in 1:dim(cl[[1]])[[1]]) {
        ...
        gene_in_cluster = cl$cluster[cl$cluster == j] # member of cluster j
        num.gene = length(gene_in_cluster)
                ...
                if (sum(tmpcol) > 0) {
                  ...
                  for (k in 1:length(tmpind)) {
                    if (missing(time.points)) {
                        if (!only_center){
                            if(spline){
                                lines(spline(tmp[tmpind[k], ], n=100), col = colo[jj])
                            }else{
                        ...
                        if (!only_center){
                            if(spline){
                                lines(spline(time.points, tmp[tmpind[k], ],n=100), col = colo[jj])
                            }else{
...
#             lines(cl[[1]][j, ], col = centre.col, lwd = centre.lwd)
              lines(spline(cl[[1]][j, ],n=100), col = centre.col, lwd = centre.lwd)
        }
        if (single) 
            return()
    }
